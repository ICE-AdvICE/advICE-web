...
function EditModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetchSubjectsPublic(); // 공개 API라 토큰 없어도 OK
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      weekDay:
        weekDayFromDate(initialValues.classDate) || // 날짜에서 요일 계산 우선
        normalizeWeekDay(initialValues.weekDay) ||
        prev.weekDay,
      classDate: initialValues.classDate
        ? initialValues.classDate.slice(5) // "YYYY-MM-DD" → "MM-DD"
        : prev.classDate,
    ...
    const fetchAssistants = async (sid) => {
      ...
      try {
        ...
        if (res?.code === "SU") {
          ...
          // 조교 설정 전
          setAssistants([]);
          setAssistantsErr("선택한 교과목에 등록된 조교 리스트가 없습니다.");
        } else {
  ...
  }, [form.classDate]); // form.weekDay 제거하여 무한 루프 방지

  // submit
  const submit = (e) => {
    e.preventDefault();
    if (submitting) return; // 🚧 버튼 더블 클릭 가드
    ...
    const payload = {
      classDate: apiDate,
      weekDay: koWeek, // 날짜로부터 계산된 요일 사용
