...
export const getczallattendRequest = async (
  ...
  try {
    ...
    if (code === "ATE") {
      console.warn(
        "출결 목록 조회: Access Token 만료됨. 토큰 재발급 시도 중..."
      );
...
export const downloadAttendanceExcel = async (
  ...
  try {
    ...
    if (code === "ATE") {
      console.warn(
        " 출결 Excel 다운로드: Access Token 만료됨. 토큰 재발급 시도 중..."
      );
      const newToken = await refreshTokenRequest(
        setCookie,
        accessToken,
  ...
  try {
    ...
    switch (code) {
      ...
      case "DUPLICATED_MAPPING_SET":
        return {
          success: false,
          message: "이미 존재하는 코딩존 매핑 번호와 교과목입니다.",
        };
...
export const getSubjectMappingList = async (
  ...
  try {
    ...
    if (code === "ATE") {
      console.warn(
        "코딩존 매핑 조회: Access Token 만료됨. 토큰 재발급 시도 중..."
      );
      const newToken = await refreshTokenRequest(
        setCookie,
        accessToken,
