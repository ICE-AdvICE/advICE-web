...
public class CodingZoneServiceImplement implements CodingZoneService {
    ...
    public ResponseEntity<? super CodingZoneRegisterResponseDto> classRegist(Integer classNum, String email) {
        try {
            ...
            CodingZoneRegister newRegisterEntity = new CodingZoneRegister(subjectId, email, userName, userStudentNum, classNum);
   ...
   @Override
    ...
    List<CodingZoneClass> classEntities = new ArrayList<>();
        ...
        try {
            ...
            if (subjectId != 1 && subjectId != 2) return GetListOfCodingZoneClassResponseDto.validationFailed();
    ...
    @Override
    public ResponseEntity<? super GetListOfCodingZoneClassForNotLogInResponseDto> getClassList2(Integer subjectId) {
        List<CodingZoneClass> classEntities = new ArrayList<>();
        try {
            // 사용자 계정이 존재하는지(로그인 시간이 초과됐는지) 확인하는 코드
            if (subjectId != 1 && subjectId != 2) return GetListOfCodingZoneClassForNotLogInResponseDto.validationFailed();
    ...
    public ResponseEntity<? super GetCountOfAttendResponseDto> getAttend(Integer subjectId, String email) {
        ...
        try{
            ...
            classEntities = codingZoneRegisterRepository.findBySubjectId(subjectId);
